...
Nfolds = np.linspace(0.0, 52.5, 10000)
PhiInit = np.array([20.0, 0.0])
...
#plt.title(r'A plot of $\phi[N]$ vs. N from $V = \frac{1}{4} \lambda\phi^{4}$.')
...
#plt.title(r'A plot of ln($\rho$) vs. N from $V = \frac{1}{4} \lambda\phi^{4}$.', y=1.02)
...
InflationCondition = (V/(H**2) - phis[:,1]**2)/3 # Inflation condition 18.3 from PDP book (=adotdot/aH^2 here).

plt.figure()
plt.plot(Nfolds, InflationCondition, 'b-', linewidth=0.25) # Inflation ends when Inflation condition = 1.
#plt.title(r'A plot of $\frac{1}{H^{2}} \left(\frac{\ddot{a}}{a}\right)$ vs. N from $V = \frac{1}{4} \lambda\phi^{4}$.', y=1.02)
plt.xlabel('No. of e-folds, N', fontsize=10)
...
### Plot of the curvature slow-roll parameter against the no. of e-folds.
eta = 12.0 / (phis[:,0])**2

plt.figure()
plt.plot(Nfolds, eta, 'b-', linewidth=0.25)
#plt.title(r'A plot of $\eta$ vs. N from $V = \frac{1}{4} \lambda\phi^{4}$.', y=1.02)
plt.xlabel('No. of e-folds, N', fontsize=10)
...
### Plot of the slope slow-roll parameter against the no. of e-folds.
epsilon = 8.0 / (phis[:,0])**2

plt.figure()
plt.plot(Nfolds, epsilon, 'b-', linewidth=0.25)
#plt.title(r'A plot of $\epsilon$ vs. N from $V = \frac{1}{4} \lambda\phi^{4}$.', y=1.02)
plt.xlabel('No. of e-folds, N', fontsize=10)
...
### Plot of a combination of phi, inflation condition, epsilon and eta.
func = lambda phi : 8/(phi**2) - 1
phi_initial_guess = 52
...
plt.annotate('End of Inflation at N = %.3f' % (Nfolds[index]), xy=(Nfolds[index], epsilon[index]), xytext=(47.5, -1.5), bbox=dict(boxstyle="square", fc="w"), arrowprops=dict(facecolor='black', shrink=0.05))
plt.annotate('Inflation condition end at N = %.3f' % (Nfolds[index2]), xy=(Nfolds[index2], InflationCondition[index2]), xytext=(49.0, -3.5), bbox=dict(boxstyle="square", fc="w"), arrowprops=dict(facecolor='black', shrink=0.05))
#plt.title(r'A plot of $\phi[N]$, $\epsilon$ and $\eta$ vs. N for $V = \frac{1}{4} \lambda\phi^{4}$.', y=1.02)
plt.xlabel('No. of e-folds, N', fontsize=12)
plt.ylabel(r'$\phi[N]$, $\epsilon$ and $\eta$ (all dimensionless)', fontsize=12)
plt.xlim([47.0, 52.5])
plt.ylim([-5.0, 6.0])
