...
for i in range(0, len(k)):
    ...
    phis2prime = (
        ...
        (3.0 * phis[index:, 1]) -
        (3.0 / phis[index:, 0]) +
        ((phis[index:, 1])**2 / (2.0 * phis[index:, 0]))
    )
    phis3prime = (
        ((3.0 * phis2prime * (phis[index:, 1])**2) / 2.0) -
        (3.0 * phis2prime) +
        ((3.0 * phis[index:, 1]) / (phis[index:, 0])**2) +
        ((phis2prime * phis[index:, 1]) / (phis[index:, 0])) -
        ((phis[index:, 1])**3 / (2.0 * (phis[index:, 0])**2))
    )
    z2primeBYz = (
        1 +
...
ax.plot(n_s_slowroll1, rRatio, 'c-', label=r'$\phi^{2}$ Data - 1st order slow roll')
ax.plot(n_s_slowroll2, rRatio, 'm-', label=r'$\phi^{2}$ Data - 2nd order slow roll')
ax.plot(xrange, yfit, '-', color=(0.8, 0.8, 0.8))
ax.plot(stuff, rRatio40, 'k-', label=r'$\phi^{2}$ Data - Quadratic Fit')
plt.annotate(
