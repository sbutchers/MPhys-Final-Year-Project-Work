...
plt.title(r'A plot of $\phi[N]$ vs. N from $V = \frac{1}{4} \lambda\phi^{4}$.', y=1.02)
...
LAMBDA = 1e-12
V = 0.25 * LAMBDA * phis[:,0]**4
Vprime = LAMBDA * (phis[:, 0]**3)
...
n_s_slowroll1 = 1.0 - ((24.0) / (phi_exit**2))
print "First order slow-roll prediction of n_s is:", n_s_slowroll1
n_s_slowroll2 = 1.0 - ((24.0) / (phi_exit**2)) - ((928.0)/(3.0 * phi_exit**4)) + ((18432.0 - (55296.0*C)) / (3.0 * phi_exit**8))
print "Second order slow-roll prediction of n_s is:", n_s_slowroll2
epsilon = 8.0/(phi_exit)**2
rRatio = 16.0 * epsilon
print "Slow-roll prediction of the tensor to scalar ratio r is:", rRatio
...
for i in range(0, len(k)):
    ### DEFINITION OF INITIAL E-FOLD NO. FOR EACH K NO. ###
    Cq = 1e3
    HubRadNeeded = k[i] / Cq
    index = find_nearest(np.sqrt(HubRadSqu), HubRadNeeded)
    n_ic = Nfolds[index]
    nFolds = Nfolds[index:]
    globals()['NFOLDS' + str(i)] = nFolds
    ...
    phis2prime = (
        ...
        (3.0 * phis[index:, 1]) -
        (12.0 / phis[index:, 0]) +
        ((2.0 * (phis[index:, 1])**2) / phis[index:, 0])
    )
    phis3prime = (
        ((3.0 * phis2prime * (phis[index:, 1])**2) / 2.0) -
        (3.0 * phis2prime) +
        ((12.0 * phis[index:, 1]) / (phis[index:, 0])**2) +
        ((4.0 * phis2prime * phis[index:, 1]) / (phis[index:, 0])) -
        ((2.0 * phis[index:, 1])**3 / (phis[index:, 0])**2)
    )
    z2primeBYz = (
        1 +
...
### All k-value scalar perturbation plot. ###
plt.figure()
for i in range(len(k)):
    plt.plot(
        globals()['NFOLDS' + str(i)],
        globals()['ScalPowSpec' + str(i)],
        linewidth = 0.25,
        label = 'k = %d' % k[i]
    )
plt.yscale('log')
...
plt.ylabel(r'Scalar power spectrum, $\Delta_{s}^{2}(k)$', fontsize=10)
plt.legend(loc="upper right", fontsize=9)
plt.savefig("ScalarPerturbations/AllScalarPerts.pdf", dpi=2000, bbox_inches='tight')
plt.close()

### All k-value tensor perturbation plot. ###
plt.figure()
for i in range(len(k)):
    plt.plot(
        globals()['NFOLDS' + str(i)],
        globals()['TensPowSpec' + str(i)],
        linewidth = 0.25,
        label = 'k = %d' % k[i]
    )
plt.yscale('log')
plt.grid('on')
plt.xlabel("No. of e-folds, N", fontsize=10)
plt.ylabel(r'Tensor power spectrum, $\Delta_{t}^{2}(k)$', fontsize=10)
plt.legend(loc="upper right", fontsize=9)
plt.savefig("TensorPerturbations/AllTensorPerts.pdf", dpi=2000, bbox_inches='tight')
plt.close()
...
## Fitting procedure. ##
popt, pcov = curve_fit(quadratic, lnK, LNsSpec40, p_0)
print popt
...
plt.savefig("ScalSpecKplt.pdf", dpi=2000, bbox_inches='tight')
plt.close()
...
def linear(x, a, b):
    return a*x + b
# User-found parameters for fit. #
P0 = [-4.189445438, 4.189445438]
# Fitting procedure #
POPT, PCOV = curve_fit(linear, stuff, rRatio40, P0)
print POPT
xInVar = (-1.0 * POPT[1])/POPT[0]
print xInVar
xrange = np.arange(0.94, 1.05, 0.001)
INDEXinvar = find_nearest(xrange, xInVar)
yfit = linear(xrange, POPT[0], POPT[1])
...
ax.plot(n_s_slowroll1, rRatio, 'c-', label=r'$\phi^{4}$ Data - 1st order slow roll')
ax.plot(n_s_slowroll2, rRatio, 'm-', label=r'$\phi^{4}$ Data - 2nd order slow roll')
ax.plot(xrange, yfit, '-', color=(0.8, 0.8, 0.8))
ax.plot(stuff, rRatio40, 'k-', label=r'$\phi^{4}$ Data - Quadratic Fit')
plt.annotate(
    '\n'.join(wrap(r'Scale intercept at $n_{s}$ = %.3f' % (xInVar), 20)),
    fontsize=10,
    xy=(xrange[INDEXinvar], 0.0),
    xytext=(0.975, 0.16),
...
plt.savefig("ns_r.pdf", dpi=2000, bbox_inches='tight')
